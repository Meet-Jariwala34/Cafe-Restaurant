...
export default function List({token}) {
  ...
  const toggleAvailability = async (item, currAvailability) => {
    try {

      if(currAvailability){
        const r = await axios.put(backendUrl+'/api/product/soldOutItem',{id : item._id},{headers:{'token':token}});
        if (r.data.success) {
            // 2. FRONTEND UI UPDATE (The Missing Step!)
            // We loop through the list and change 'isAvailable' ONLY for the item we clicked
            setList((prevList) => 
                prevList.map((prevItem) => 
                    prevItem._id === item._id 
                        ? { ...prevItem, isAvailable: !currAvailability } 
                        : prevItem
                )
            );}
      }else{
        const r = await axios.put(backendUrl+'/api/product/notSoldOutItem',{id : item._id},{headers:{'token':token}});
        if (r.data.success) {
            // 2. FRONTEND UI UPDATE (The Missing Step!)
            // We loop through the list and change 'isAvailable' ONLY for the item we clicked
            setList((prevList) => 
  ...
  const fetchList = async () =>{
    try {
      ...
      const res = await axios.get(backendUrl+'/api/product/listMenu');
  ...
  return (
    <div className='h-[90vh] w-[75vw] px-5 gap-2 overflow-y-auto flex-nowrap z-0'>
      <p className='text-2xl text-center roboto'>You have total {list.length} Dishes</p>
        {
          list.map((item,index)=>(
            <div key={index} className='my-4 h-[12vh] w-full p-1.5 flex items-center bg-[#1a1b1c] rounded-md justify-between'>
              <div><img className='h-[10vh] w-12vh object-cover rounded-md' src={item.image} alt="Food image"/></div>
