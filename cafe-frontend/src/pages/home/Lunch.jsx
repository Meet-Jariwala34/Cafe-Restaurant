...
import food3 from '../../assets/menu-image-1.jpg';
...
export default function Lunch() {
  ...
  return (
    <>
    <h2 className='roboto-condensed text-3xl text-center m-5'>Lunch</h2>
        <div className='md:h-[80vh] h-[100vh] ease-in w-[100%] md:p-[5vmax] justify-between flex flex-col md:flex-row'>
          ...
          <div className='md:h-full h-[45%] w-[100%] md:w-[40%] flex flex-col justify-between'>
            {/* single food item */}
            {
          list.filter(item => item.category === "lunch").filter(item => item.isAvailable === true).slice(0,3).map((item,index)=>
          ...
          <div className='md:h-full h-[45%] w-[100%] md:w-[40%] flex flex-col justify-between'>
            {/* single food item */}
            {
          list.filter(item => item.category === "lunch").filter(item => item.isAvailable === true).slice(3,6).map((item,index)=>
            <div className='h-25 w-full flex cursor-pointer'>
          {/* img  */}
          <div className='h-full'><img className='h-full w-25 rounded-md object-cover' src={item.image} alt="food1" /></div>
          {/* detail */}
